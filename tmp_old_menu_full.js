...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const categories = [
        { id: "beverage", name: "Beverage", icon: "Γÿò" },
        { id: "chaat", name: "Chaat", icon: "≡ƒÑƒ" },
        { id: "street-food", name: "Street Food", icon: "≡ƒÑÖ" },
        { id: "paratha", name: "Paratha", icon: "≡ƒ½ô" },
        { id: "south-indian", name: "South Indian", icon: "≡ƒî«" },
        { id: "thali", name: "Thali", icon: "≡ƒì▒" },
        { id: "soup", name: "Soup", icon: "≡ƒÑú" },
        { id: "bread", name: "Bread", icon: "≡ƒì₧" },
        { id: "quick-bites", name: "Quick Bites", icon: "≡ƒì£" }
    ];
                ...
                html += `
                    <div class="dish-card ${!item.available ? 'out-of-stock' : ''}" style="${!item.available ? 'opacity: 0.7;' : ''}">
                        <div class="dish-info">
                            ...
                            <div class="dish-price">Γé╣${item.price}</div>
                            <p class="dish-desc">Authentic taste made with pure ingredients.</p>
                            ${item.bestseller ? `<div class="dish-tags"><span class="dish-tag">Bestseller Γ¡É</span></div>` : ''}
                        </div>
        ...
        cart.forEach(item => {
            html += `
                <div class="order-modal-item">
                    ...
                    <div class="item-total">Γé╣${item.price * item.qty}</div>
                </div>
            `;
        });
...
function showToast(message) {
    ...
    toast.innerHTML = `<span style="color:#34A853">Γ£ô</span> ${message}`;
